...
(function () {
  ...
  const BAR_LINKS = [
    { id: "barEnergy", url: "/gym.php",               title: "Go to the Gym", fullCls: "tdh-full-energy" },
    { id: "barNerve",  url: "/loader.php?sid=crimes", title: "Go to Crimes",  fullCls: "tdh-full-nerve" },
  ];

  // Flag a bar as full (current >= max) so CSS can pulse it.
  function updateBarFullState(bar, fullCls) {
    // try visible text first, then aria-labels (Torn's markup varies)
    let text = bar.textContent || "";
    let m = text.match(/(\d[\d,]*)\s*\/\s*(\d[\d,]*)/);
    if (!m) {
      for (const el of bar.querySelectorAll("[aria-label], [title]")) {
        const t = (el.getAttribute("aria-label") || "") + " " + (el.getAttribute("title") || "");
        m = t.match(/(\d[\d,]*)\s*\/\s*(\d[\d,]*)/);
        if (m) break;
      }
    }
    if (!m) { console.debug("[TDH] no cur/max found in", bar.id); return; }
    const cur = parseInt(m[1].replace(/,/g, ""), 10);
    const max = parseInt(m[2].replace(/,/g, ""), 10);
    const full = max > 0 && cur >= max;
    bar.classList.toggle(fullCls, full);
    console.debug("[TDH]", bar.id, cur + "/" + max, full ? "FULL → glow on" : "");
  }
  ...
  function findBar(id) {
    return (
      ...
      document.querySelector(`[id^="${id}" i], [class*="${id}" i]`)
    );
  }
  ...
  function initBarLinks() {
    ...
    for (const cfg of BAR_LINKS) {
      const bar = findBar(cfg.id);
      if (bar) { wireBar(bar, cfg); wired++; }
      else console.debug("[TDH] sidebar bar not found:", cfg.id);
    }
