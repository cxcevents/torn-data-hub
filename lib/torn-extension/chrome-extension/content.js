...
(function () {
  ...
  function applyTheme() {
    ...
    for (const el of Object.values(rootEls)) {
  ...
  function saveLayout() {
    for (const [name, el] of Object.entries(rootEls)) {
      if (el && el.parentElement) {
        layout[name] = Array.prototype.indexOf.call(el.parentElement.children, el);
      }
    }
  ...
  function scheduleRestore() {
    ...
    queueMicrotask(() => {
      ...
      const entries = Object.entries(rootEls)
        .filter(([n, el]) => el && el.parentElement && layout[n] !== undefined)
        .sort((a, b) => layout[a[0]] - layout[b[0]]);
      for (const [name, el] of entries) {
        const parent = el.parentElement;
        const others = Array.from(parent.children).filter((k) => k !== el);
        parent.insertBefore(el, others[layout[name]] ?? null);
      }
    });
  }
  ...
  function initDrag() {
    document.addEventListener("pointerdown", (e) => {
      ...
      const move = (ev) => {
        ...
        if (ref !== root && ref !== root.nextSibling) parent.insertBefore(root, ref);
      };
  ...
  const GYMEFF_CACHE  = "tdh_gymeff_cache_v2"; // v2: attack credits added
  ...
  async function computeGymEff() {
    ...
    // War hits = ranked war or during a chain; they never count against the score.
    const attacks = (await fetchAttacks(from)).filter((a) => a.attacker_id === myId);
    let warHits = 0, otherHits = 0;
    for (const a of attacks) {
      const isWar = a.ranked_war === 1 || (a.modifiers?.war ?? 1) > 1 || (a.chain ?? 0) >= 10;
  ...
  function renderGymEff(el, s, overflow7d) {
    ...
    const warRow = s.warHits
      ? `<div class="tdh-gym-row"><span>War &amp; chain hits (excluded)</span><span>${fmtNum(s.warE)}E · ${s.warHits} hits</span></div>` : "";
    const otherAtkRow = s.otherHits
      ? `<div class="tdh-gym-row"><span>Other attacks (excluded)</span><span>${fmtNum(s.otherE)}E · ${s.otherHits} hits</span></div>` : "";
    ...
    el.innerHTML = `
      ...
      <div class="tdh-card-body">
        ...
        ${overflowRow}
        <div class="tdh-gym-note">Attacks never count against the score — war/chain hits and outside hits are both credited at 25E. Unaccounted energy = items/other or regen lost to a full bar (stacking shows up there; that's expected before wars). Overflow tracking is exact from install onward.</div>
      </div>
    `;
  }
  ...
  function renderEff() {
    ...
    const effEl = mountRoot("eff", "after");
    if (!effEl) return;
