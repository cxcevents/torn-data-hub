...
(function () {
  ...
  const feat = {
    xanax: true, refills: true, effstats: true, gym: true, barlinks: true,
    nag: true, warOnline: true, warOkay: true, warGlow: true,
  };
  ...
  function updateRankedWarOnlineCounts() {
    ...
    factionNames.slice(0, 2).forEach((nameBox, index) => {
      ...
      const memberRows = Array.from(panel.querySelectorAll("ul[class*='members'] > li"));
      const okay = memberRows.filter((row) => {
        const status = row.querySelector(":scope > [class~='status']");
        return (status?.textContent || "").trim().toLowerCase() === "okay";
      }).length;
      ...
      let okayIndicator = nameBox.querySelector(".tdh-war-okay");
      if (!feat.warOkay) {
        okayIndicator?.remove();
        return;
      }
      if (!okayIndicator) {
        okayIndicator = document.createElement("div");
        okayIndicator.className = "tdh-war-okay";
        (indicator || score).insertAdjacentElement("afterend", okayIndicator);
      }
      const okayLabel = `${okay}/${memberRows.length} okay`;
      if (okayIndicator.textContent !== okayLabel) okayIndicator.textContent = okayLabel;
      okayIndicator.setAttribute(
        "title",
        `${okay} of ${memberRows.length} faction members currently have Okay status`,
      );
    });

    if (feat.warGlow && indicators.length === 2) {
  ...
  function findTornToolsWarFilter() {
    const titles = document.querySelectorAll(".title .text, [class~='title'] [class~='text']");
    for (const title of titles) {
      if ((title.textContent || "").trim() !== "Ranked War Filter") continue;
      let panel = title.parentElement;
      for (let i = 0; i < 7 && panel; i++, panel = panel.parentElement) {
        const text = panel.textContent || "";
        if (text.includes("Activity") && text.includes("Status") && text.includes("Level Filter")) {
          return panel;
        }
      }
    }
  ...
  // When TornTools is present, add our controls to its ranked-war filter panel.
  // The controls write to the same feature store as the full TDH preferences page.
  function mountTornToolsPillControls() {
    const panel = findTornToolsWarFilter();
    if (!panel || panel.querySelector(".tdh-tt-pill-controls")) return;

    const headings = {};
    const wantedHeadings = ["Activity", "Status", "Level Filter", "Stats Estimates"];
    for (const el of panel.querySelectorAll("div, span, label, strong")) {
      const text = (el.textContent || "").trim();
      if (wantedHeadings.includes(text) && !headings[text]) headings[text] = el;
    }
    if (wantedHeadings.some((heading) => !headings[heading])) return;

    // Find the lowest container shared by all four TornTools filter groups.
    // This keeps TDH inside TornTools' collapsible body and avoids accidentally
    // appending it below the filter row when a group's wrapper is too broad.
    let filterRow = headings.Status;
    while (
      filterRow &&
      !wantedHeadings.every((heading) => filterRow.contains(headings[heading]))
    ) {
      filterRow = filterRow.parentElement;
    }
    if (!filterRow || filterRow === panel) return;

    let statusGroup = headings.Status;
    while (statusGroup.parentElement && statusGroup.parentElement !== filterRow) {
      statusGroup = statusGroup.parentElement;
    }
    if (statusGroup.parentElement !== filterRow) return;

    const controls = document.createElement("div");
    controls.className = "tdh-tt-pill-controls";
    controls.innerHTML = `
      <strong>TDH Pills</strong>
      <label><input type="checkbox" data-tdh-feature="warOnline"> Online count</label>
      <label><input type="checkbox" data-tdh-feature="warOkay"> Okay status</label>
      <label><input type="checkbox" data-tdh-feature="warGlow"> Leader glow</label>
    `;
    for (const input of controls.querySelectorAll("[data-tdh-feature]")) {
      input.checked = feat[input.dataset.tdhFeature] !== false;
      input.addEventListener("change", () => {
        feat[input.dataset.tdhFeature] = input.checked;
        chrome.storage.local.get("tdh_features", (stored) => {
          const next = Object.assign({}, stored.tdh_features || {}, {
            [input.dataset.tdhFeature]: input.checked,
          });
          chrome.storage.local.set({ tdh_features: next });
        });
        updateRankedWarOnlineCounts();
      });
    }

    statusGroup.insertAdjacentElement("afterend", controls);
  }
  ...
  chrome.storage.onChanged?.addListener((changes, area) => {
    if (area !== "local" || !changes.tdh_features?.newValue) return;
    Object.assign(feat, changes.tdh_features.newValue);
    for (const input of document.querySelectorAll(".tdh-tt-pill-controls [data-tdh-feature]")) {
      input.checked = feat[input.dataset.tdhFeature] !== false;
    }
    updateRankedWarOnlineCounts();
  });
