...
// @version      2.3.0
// @description  Injects Xanax and Refills cards above the Battle Stats card on the Torn homepage, styled to match Torn Data Hub dashboard
...
(function () {
  ...
  async function fetchMain() {
    if (!apiKey) return;
    try {
      lastData  = await tornApi("cooldowns,refills");
      fetchedAt = Date.now();
      render();
    } catch (e) {
  ...
  async function fetchXanaxLog() {
    if (!apiKey) return;
    try {
      // Torn's log API caps at ~100 entries per call, so paginate back to midnight TCT
      const from = midnightUnix();
      let count = 0;
      let to = null;
      for (let page = 0; page < 10; page++) {
        const json    = await tornApi("log", `&from=${from}${to !== null ? `&to=${to}` : ""}`);
        const entries = Object.values(json.log ?? {});
        if (entries.length === 0) break;
        count += entries.filter((e) => e.title && e.title.toLowerCase().includes("xanax")).length;
